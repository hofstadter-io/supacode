...
const getPageBySlug = async (content: string, slug: string): Promise<Post> => {
	...
	var pageModule = null;
	try {
		pageModule = await import(`@/content/${content}/${slug}.mdx`);
	} catch {
		pageModule = await import(`@/content/${content}/${slug}/index.mdx`);
	}
...
export const getAllPages = async (content: string) => {
  ...
  pages.sort((page1, page2) => (page1.date > page2.date ? -1 : 1));
