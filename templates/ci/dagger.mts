...
async function publish(client: Client, source: Directory) {
	...
	// await exportImage(image, `{% .App.name %}:${version}`)
	await image.publish(`${opts.registry}/{% .App.name %}-server:${opts.version}`)
	await nginx.publish(`${opts.registry}/{% .App.name %}-nginx:${opts.version}`)
}
...
function makeImage(client: Client, source: Directory) {
	...
	const build = base.pipeline("build")
		...
		.withExec(["npm", "exec", "prisma", "generate"])
		{% end %}
